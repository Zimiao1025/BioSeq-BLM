...
def plot_roc(true_labels_list, predict_prob_list, tags, figure_name):
    plt.figure(0)
    count = 0
    ...
    for true_labels, predict_prob in zip(true_labels_list, predict_prob_list):
...
def plot_pr(true_labels_list, predict_prob_list, tags, figure_name):
    plt.figure(0)
    count = 0
    color_list = ['crimson', 'navy', 'teal', 'darkorange', 'purple', 'gray',
                  'green', 'dodgerblue', 'gold', 'lightcoral']
...
def extract_data(path_name, tags):
    ...
    return true_labels_list, predict_prob_list
...
def main():
    target_path = "../results/target_res/"
    metric_tup = traverse_folder(target_path, "AUC")
    tags, new_tags = make_tag(metric_tup, 10)

    true_labels_list, predict_prob_list = extract_data(target_path, tags)
    print(len(true_labels_list))
    print(len(predict_prob_list))
    # exit()

    fig_path = "../results/figure/"
    plot_roc(true_labels_list, predict_prob_list, new_tags, fig_path + "res_roc.png")
    plot_pr(true_labels_list, predict_prob_list, new_tags, fig_path + "res_pr.png")
