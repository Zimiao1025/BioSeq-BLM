...
class MotifDCNN(torch.nn.Module):
    def __init__(self, hidden_dim, n_layer, fea_dim, n_class, prob, args):
        super(MotifDCNN, self).__init__()
        ...
        self.kernel = motifs
        self.rnn = nn.LSTM(len(motifs) * 4, hidden_dim, n_layer,
                           bidirectional=True,
                           batch_first=True,
                           dropout=prob)
    ...
    def extract_feature(self, x):
        ...
        out = torch.cat([motif_layer, cnn_layer], 1).unsqueeze(1)  # size: [50 x 1 x 1204] 1204是模体个数的四倍
        out, _ = self.rnn(out)  # torch.Size([50, 1, 256*2])
        out = out[:, -1, :]
        out = self.fc(out)
        return out
